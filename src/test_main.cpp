...
void dual_println(T &&... items) {
    dual_print(std::forward<T>(items)..., synced_stream::endl);
}
...
Antares::concurrency_t count_unique_threads() {
    const Antares::concurrency_t num_tasks = pool.get_thread_count() * 2;
    std::vector<std::thread::id> thread_IDs(num_tasks);
    std::unique_lock<std::mutex> total_lock(total_mutex);
    Antares::concurrency_t total_count = 0;
    ...
    return static_cast<Antares::concurrency_t>(std::unique(thread_IDs.begin(), thread_IDs.end()) - thread_IDs.begin());
}
...
void check_wait_for_tasks() {
    const Antares::concurrency_t n = pool.get_thread_count() * 10;
    std::unique_ptr<std::atomic<bool>[]> flags = std::make_unique<std::atomic<bool>[]>(n);
    for (Antares::concurrency_t i = 0; i < n; ++i)
        pool.push_task(
                [&flags, i] {
 ...
 */
template<typename T>
void check_parallelize_loop_no_return(const int64_t random_start, T random_end, Antares::concurrency_t num_tasks,
                                      bool use_push = false) {
    if (random_start == random_end)
        ++random_end;
    dual_println("Verifying that ", use_push ? "push_loop()" : "parallelize_loop()", " from ", random_start, " to ",
                 random_end, " with ", num_tasks, num_tasks == 1 ? " task" : " tasks", " modifies all indices...");
    ...
    if (use_push) {
        ...
        if (random_start == 0) {
            auto result = pool.parallelize_loop(random_end, loop, num_tasks);
            for (auto &f: result) {
                f.wait();
            }
        } else {
            auto result = pool.parallelize_loop(random_start, random_end, loop, num_tasks);
            for (auto &f: result) {
                f.wait();
            }
        }
...
[[nodiscard]] std::conditional_t<std::is_void_v<R>, void, std::vector<R>>
getFutureVector(std::vector<std::future<R>, Allocator> futures) {
    if constexpr (std::is_void_v<R>) {
        for (size_t i = 0; i < futures.size(); ++i)
            futures[i].get();
        return;
    } else {
        std::vector<R> results(futures.size());
        for (size_t i = 0; i < futures.size(); ++i)
 ...
 */
void
check_parallelize_loop_return(const int64_t random_start, int64_t random_end, const Antares::concurrency_t num_tasks) {
    if (random_start == random_end)
        ++random_end;
    dual_println("Verifying that parallelize_loop() from ", random_start, " to ", random_end, " with ", num_tasks,
                 num_tasks == 1 ? " task" : " tasks", " correctly sums all indices...");
    ...
    const std::vector<int64_t> sums_vector = (random_start == 0) ? getFutureVector(
            pool.parallelize_loop(random_end, loop,
                                  num_tasks))
                                                                 : getFutureVector(
                    pool.parallelize_loop(random_start, random_end, loop,
                                          num_tasks));
    int64_t sum = 0;
...
void check_parallelize_loop() {
    ...
    std::uniform_int_distribution<Antares::concurrency_t> task_dist(1, pool.get_thread_count());
    ...
    auto result = pool.parallelize_loop(index, index, [&flag](const int64_t, const int64_t) { flag = false; });
    for (auto &r: result) {
        r.wait();
    }
    check(flag);
...
void check_task_monitoring() {
    Antares::concurrency_t n = std::min<Antares::concurrency_t>(std::thread::hardware_concurrency(), 4);
    ...
    for (Antares::concurrency_t i = 0; i < n * 3; ++i)
        pool.push_task(
                [&release, i] {
                    while (!release[i])
    ...
    dual_println("After submission, should have: ", n * 3, " tasks total, ", n, " tasks running, ", n * 2,
                 " tasks queued...");
    dual_print("Result: ", pool.get_tasks_total(), " tasks total, ", pool.get_tasks_running(), " tasks running, ",
    ...
    for (Antares::concurrency_t i = n; i < n * 2; ++i)
        release[i] = true;
    std::this_thread::sleep_for(sleep_time);
    ...
    for (Antares::concurrency_t i = n * 2; i < n * 3; ++i)
        release[i] = true;
    std::this_thread::sleep_for(sleep_time);
...
void check_exceptions() {
    ...
    caught = false;
    std::vector<std::future<void>> my_future2;
    my_future2.push_back(pool.submit(throws));
    my_future2.push_back(pool.submit(throws));
    try {
        for (auto &r: my_future2) {
            r.get();
        }
    }
    catch (const std::exception &e) {
 ...
 */
void check_vector_of_size(size_t vector_size, Antares::concurrency_t num_tasks) {
    ...
    auto result = pool.parallelize_loop(
            0, vector_size,
 ...
 */
void print_timing(const Antares::concurrency_t num_tasks, const std::pair<double, double> &mean_sd) {
    if (num_tasks == 0)
 ...
 */
void print_speedup(const std::vector<double> &timings, const Antares::concurrency_t try_tasks[]) {
    ...
    const Antares::concurrency_t num_tasks = try_tasks[min_el - std::begin(timings)];
    dual_println("Maximum speedup obtained by multithreading vs. single-threading: ", max_speedup, "x, using ",
                 num_tasks, " tasks.");
}
...
std::pair<double, double> analyze(const std::vector<std::chrono::milliseconds::rep> &timings) {
    ...
    double variance = 0;
    for (size_t i = 0; i < timings.size(); ++i)
        variance += (static_cast<double>(timings[i]) - mean) * (static_cast<double>(timings[i]) - mean) /
...
struct timer {
    ...
    using timeduration = decltype(std::declval<timepoint>() - std::declval<timepoint>());
...
void check_performance() {
    ...
    const Antares::concurrency_t thread_count = pool.get_thread_count();
    dual_println("Using ", thread_count, " threads.");

    // Define the number of tasks to try in each run of the test (0 = single-threaded).
    const Antares::concurrency_t try_tasks[] = {0, thread_count / 4, thread_count / 2, thread_count, thread_count * 2,
                                                thread_count * 4};
    ...
    dual_println(
            "Determining the number and size of vectors to generate in order to achieve an approximate mean execution time of ",
            target_ms, " ms with ", thread_count, " tasks...");
    ...
    num_vectors = thread_count * static_cast<size_t>(std::llround(
            static_cast<double>(num_vectors) * static_cast<double>(target_ms) / static_cast<double>(tmr.ms()) /
            thread_count));
...
int main() {
    const std::string log_filename = "lockfree_thread_pool_test-" + get_time() + ".log";
    if (output_log)
        log_file.open(log_filename);

    dual_println("lockfree-threadpool: a fast, lightweight, and easy-to-use C++17 thread pool library, based on https://github.com/bshoshany/thread-pool(c) 2022 Barak Shoshany (baraksh@gmail.com) (http://baraksh.com) and https://github.com/cameron314/concurrentqueue");
    dual_println("antares0982@gmail.com");
    dual_println("GitHub: https://github.com/Antares0982/lockfree-threadpool\n");
